...
async function renderHome(v) {
  ...
  v.innerHTML = `
    ...
    <div class="section" style="padding-top:0"><div class="stack">
      ${lb ? `<a class="card row" href="#/biblia/${last.book}/${last.chapter}"><div class="ico">${icon('history')}</div><div class="grow"><div class="small muted">Continuar lendo</div><b>${esc(lb.name)} ${last.chapter}${store.readPos(last.book, last.chapter) > 1 ? ` · versículo ${store.readPos(last.book, last.chapter)}` : ''}</b></div>${icon('chevR')}</a>` : ''}
