...
- **Narração gravada** (padrão quando existe): MP3 por capítulo gerado de antemão com as vozes neurais Kokoro pt-br (Apache 2.0): Alex `pm_alex`, Santa `pm_santa` e Dora `pf_dora`, escolhidas no painel de áudio em estilo de narração (introdução do capítulo, pausas entre versículos), hospedado no branch `audio` do repositório e servido pelo jsDelivr; `data/audio.json` diz quais capítulos existem, e cada capítulo traz um JSON com o tempo de cada versículo para o destaque acompanhar a voz. Geração: `tools/gen_audio.py`.
