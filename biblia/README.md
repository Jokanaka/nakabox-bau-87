...
- **Narração gravada** (padrão quando existe): MP3 por capítulo gerado de antemão com a voz neural Kokoro `pm_alex` (pt-br, Apache 2.0) em estilo de narração (introdução do capítulo, pausas entre versículos), hospedado no branch `audio` do repositório e servido pelo jsDelivr; `data/audio.json` diz quais capítulos existem, e cada capítulo traz um JSON com o tempo de cada versículo para o destaque acompanhar a voz. Geração: `tools/gen_audio.py`.
