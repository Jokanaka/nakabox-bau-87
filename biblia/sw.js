...
const SHELL_CACHE = 'bc-v17-shell';   // casca do app: mude a cada versão do código
...
const SHELL = [
  ...
  './js/features.js', './js/liturgy.js', './js/audio.js', './js/cloudtts.js', './js/piper-worker.js', './js/missa.js', './js/voz.js', './js/profiles.js',
...
self.addEventListener('fetch', (e) => {
  ...
  // manifesto da narração gravada: rede primeiro, para vozes e livros novos aparecerem sem reinstalar o app
  if (url.pathname.endsWith('/data/audio.json')) {
    e.respondWith(
      fetch(req).then((res) => {
